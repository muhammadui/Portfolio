  ...
  (c.DEFAULTS = {
    ...
    (c.prototype = {
      initialize: function () {
        ...
        if ("function" == typeof this.options.fontSize)
          ...
          this.sizeGenerator = function (a, b, c) {
            ...
            return (
              Math.round(
                e + ((1 * (d - e)) / (this.options.steps - 1)) * (c - 1)
              ) + "px"
            );
          };
        ...
        (this.data.angle = 6.28 * Math.random()),
          ...
          (this.data.namespace =
            (this.$element.attr("id") ||
              Math.floor(1e6 * Math.random()).toString(36)) + "_word_"),
          this.$element.addClass("jqcloud"),
          "static" === this.$element.css("position") &&
            this.$element.css("position", "relative"),
          this.createTimeout(a.proxy(this.drawWordCloud, this), 10),
          this.options.autoResize &&
            a(window).on("resize", b(this.resize, 50, this));
      },
      ...
      overlapping: function (a, b) {
        return Math.abs(2 * a.left + a.width - 2 * b.left - b.width) <
          a.width + b.width &&
          Math.abs(2 * a.top + a.height - 2 * b.top - b.height) <
            a.height + b.height
          ? !0
          : !1;
      },
      ...
      drawWordCloud: function () {
        ...
        if (
          ...
          for (a = 0, b = this.word_array.length; b > a; a++)
            this.word_array[a].weight = parseFloat(
              this.word_array[a].weight,
              10
            );
          if (
            ...
            (this.data.min_weight = this.word_array[
              this.word_array.length - 1
            ].weight),
            (this.data.colors = []),
            this.colorGenerator)
          )
          ...
          if (this.options.delay > 0) this.drawOneWordDelayed();
            ...
            "function" == typeof this.options.afterCloudRender &&
      ...
      drawOneWord: function (b, c) {
          ...
          c.attr = a.extend({}, c.html, { id: g }),
            this.data.max_weight != this.data.min_weight &&
              (l =
                Math.round(
                  (1 *
                    (c.weight - this.data.min_weight) *
                    (this.options.steps - 1)) /
                    (this.data.max_weight - this.data.min_weight)
                ) + 1),
            d = a("<span>").attr(c.attr),
            this.options.classPattern &&
              d.addClass(this.options.classPattern.replace("{n}", l)),
            this.data.colors.length && d.css("color", this.data.colors[l - 1]),
            this.data.sizes.length &&
              d.css("font-size", this.data.sizes[l - 1]),
            this.options.template
              ? d.html(this.options.template(c))
              : c.link
              ? ("string" == typeof c.link && (c.link = { href: c.link }),
                this.options.encodeURI &&
                  (c.link.href = encodeURI(c.link.href).replace(/'/g, "%27")),
                d.append(a("<a>").attr(c.link).text(c.text)))
              : d.text(c.text),
            c.handlers && d.on(c.handlers),
            this.$element.append(d),
            e = { width: d.outerWidth(), height: d.outerHeight() },
          ...
          if ("rectangular" === this.options.shape)
            switch (
              ...
              j * this.data.step >
                (1 + Math.floor(k / 2)) *
                  this.data.step *
                  ((k % 4) % 2 === 0 ? 1 : this.data.aspect_ratio) &&
                ((j = 0), k++),
              k % 4)
            ) {
              case 1:
                e.left +=
                  this.data.step * this.data.aspect_ratio + 2 * Math.random();
                break;
              case 2:
            ...
            (i += this.data.step),
              ...
              (e.left =
                this.options.center.x * this.options.width -
                e.width / 2 +
                i * Math.cos(h) * this.data.aspect_ratio),
              (e.top =
                this.options.center.y * this.options.height +
                i * Math.sin(h) -
                e.height / 2);
        ...
        return this.options.removeOverflowing &&
          (e.left < 0 ||
            e.top < 0 ||
            e.left + e.width > this.options.width ||
            e.top + e.height > this.options.height)
          ? void d.remove()
          : (this.data.placed_words.push(e),
            void (
              "function" == typeof c.afterWordRender &&
              c.afterWordRender.call(d)
            ));
      },
    ...
    (a.fn.jQCloud = function (b, d) {
      ...
      return this.each(function () {
        ...
        if (g || "destroy" !== b)
          if (g)
            "string" == typeof b &&
              g[b].apply(g, Array.prototype.slice.call(e, 1));
          else {
